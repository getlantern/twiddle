...
func extData(rec []byte, want uint16) []byte {
	if len(rec) < 6 { return nil }
	b := rec[5:]
	p := 4 + 2 + 32
	if p >= len(b) { return nil }
	p += 1 + int(b[p])
	if p+2 > len(b) { return nil }
	...
	for p+4 <= end && p+4 <= len(b) {
		id := binary.BigEndian.Uint16(b[p:p+2]); ln := int(binary.BigEndian.Uint16(b[p+2:p+4]))
		if id == want && p+4+ln <= len(b) { return b[p+4 : p+4+ln] }
		p += 4 + ln
	}
	return nil
}
...
func run(host string) {
	...
	for i := 0; i < 2; i++ {
		...
		if err := c.Handshake(); err != nil { fmt.Println("  handshake:", err); raw.Close(); return }
	...
	if full == nil || resumed == nil { fmt.Println("  incomplete"); return }
	...
	if psk, ok := re[0x0029]; ok {
		...
		if len(ro) > 0 && ro[len(ro)-1] == 0x0029 { fmt.Printf("  [LAST extension, as required]") }
		fmt.Println()
		if d := extData(resumed, 0x0029); d != nil { dumpPSK(d) }
	} else {
...
func main() {
	hosts := os.Args[1:]
	if len(hosts) == 0 { hosts = []string{"www.google.com", "www.cloudflare.com", "www.microsoft.com", "github.com"} }
	for _, h := range hosts {
		fmt.Printf("\n=== %s\n", h)
		run(h)
	}
