...
func main() {
	...
	for seen < 8 {
		...
		if err != nil { continue }
		h := make([]byte, 5)
		if _, e := io.ReadFull(c, h); e != nil { c.Close(); continue }
		b := make([]byte, int(binary.BigEndian.Uint16(h[3:5])))
		io.ReadFull(c, b); c.Close()
		if h[0] != 0x16 { continue }
		p := 4 + 2 + 32
		p += 1 + int(b[p])
		...
		for p+4 <= end && p+4 <= len(b) {
			id := binary.BigEndian.Uint16(b[p:p+2]); ln2 := int(binary.BigEndian.Uint16(b[p+2:p+4]))
			if id == 0xfe0d { ech = ln2 }
			if id == 0 && ln2 > 5 { sni = string(b[p+9 : p+4+ln2]) }
			p += 4 + ln2
		}
		if sni == "" { continue }
		seen++
